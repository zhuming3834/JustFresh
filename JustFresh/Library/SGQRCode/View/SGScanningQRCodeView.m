...
/** 定时器和动画的时间 */
static CGFloat const timer_animation_Duration = 0.02; // 0.05 -> 0.02
...
//    self.timer =[NSTimer scheduledTimerWithTimeInterval:timer_animation_Duration target:self selector:@selector(animation_line_action) userInfo:nil repeats:YES];
    ...
    dispatch_time_t start =dispatch_time(DISPATCH_TIME_NOW, (int64_t)(timer_animation_Duration * NSEC_PER_SEC));
    uint64_t interval = (uint64_t)(timer_animation_Duration *NSEC_PER_SEC);
    dispatch_source_set_timer(self.timer, start, interval,0);
    
    // 设置回调
    dispatch_source_set_event_handler(self.timer, ^{
        [self animation_line_action];
    });
    // 启动定时器
...
- (void)animation_line_action {
    ...
    if (flag) {
        ...
        if (_animation_line.frame.origin.y >= scanContent_Y) {
            ...
            if (_animation_line.frame.origin.y >= scanContent_MaxY) {
                frame.origin.y = scanContent_Y;
                _animation_line.frame = frame;
                flag = YES;
            } else {
